...
# Проверка библиотек
try:
    import time, random, datetime, asyncio, sys, wikipedia, logging, aiohttp, pyrogram, os, wget, bs4, requests, gtts, \
        colorama, youtube_dl, db0mb3r, configparser
...
import time, random, datetime, asyncio, sys, wikipedia, requests, json, colorama, requests, youtube_dl, subprocess, \
    configparser
...
⇛ bomber - Запуск Бомбера (Сайт)
⇛ bbomber [Номер без знака +] - Запуск бомбера (боты)
⇛ sbomber - Завершение роботы бомбера
...
⇛ tagall - Призыв всех участников
⇛ id - Айди
⇛ info - Информация
⇛ infofull - Полная информация
⇛ qr [Текст] - Создание QR-Кода с вашим текстом
⇛ time - Текущее время
⇛ ladder - текст лесенкой (п пр при прив привет)
⇛ webshot [Ссылка] - Скриншот сайта
⇛ autoread - Авто-чтение (нет уведомлений с этого чата)
⇛ spam [Кол-во смс] [Текст сообщения] - Спам
⇛ yt [ссылка] - Скачивание и отправка видео (ютуб, тикток, лайк, инста)
⇛ myt [ссылка] - Скачивание и отправа звука с видео (ютуб, тикток, лайк, инста)
⇛ q [Ответ] - Сделать цитату (Стикер с текстом пользователя)
⇛ spamban [Ответ] - Проверка ограничений
⇛ voice [Текст] - Текст в голосовое
⇛ text [Ответ на голосовое] - Голосовое сообщение в текст
⇛ cl [Текст] - Шифровка текста [Только пользователи CLIP]
...
⇛ mute - Мут
⇛ unmute - Размут
⇛ admin - Выдача прав админа
⇛ unadmin - Разжалование Админа
⇛ invite (Юзейрнейм - @) - Пригласить в чат
⇛ kickall - Удаление всех с чата
⇛ kickall hide - Удаление всех (скрыто)
⇛ leave - Выйти с чата
⇛ pin - Закрепить
⇛ unpin - Открепить

[Репутация, для повышения попросите 2 человека написать вам в ответ сообщение "+"]""", disable_web_page_preview=True)
...
async def ping(client: Client, message: Message):
    ...
    await utils.answer(message, strings("running", message))
    ...
    for server in args:
        try:
            ...
            pass
    results = await utils.run_sync(speedtest, servers)
    ret = strings("results", message) + "\n\n"
    ret += strings("results_download", message).format(round(results["download"] / 2 ** 20, 2)) + "\n"
    ret += strings("results_upload", message).format(round(results["upload"] / 2 ** 20, 2)) + "\n"
    ret += strings("results_ping", message).format(round(results["ping"], 2)) + "\n"
